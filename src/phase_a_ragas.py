...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    def _val(x, attr):
        v = getattr(x, attr, None) if hasattr(x, attr) else x.get(attr, None)
        try:
            val_f = float(v) if v is not None else 0.0
            return 0.0 if math.isnan(val_f) else val_f
        except (ValueError, TypeError):
            return 0.0

    has_nan = any(
        math.isnan(float(getattr(pq, "faithfulness", 0.0) if hasattr(pq, "faithfulness") else pq.get("faithfulness", 0.0)))
        for pq in per_q
    ) if per_q else True

    if has_nan or not per_q:
        from src.m4_eval import _compute_fallback_metrics
        raw = _compute_fallback_metrics(questions, ans_texts, contexts, ground_truths)
        per_q = raw.get("per_question", [])

    results = []
    for a, pq in zip(answers, per_q):
        f = _val(pq, "faithfulness")
        ar = _val(pq, "answer_relevancy")
        cp = _val(pq, "context_precision")
        cr = _val(pq, "context_recall")
...
def bottom_10(results: list[RagasResult]) -> list[dict]:
    ...
    for i, r in enumerate(bottom):
        diag, fix = DIAGNOSTIC_TREE.get(r.worst_metric, ("Unknown error", "Review pipeline configuration"))
        output.append({
            "rank": i + 1,
...
def cluster_analysis(results: list[RagasResult]) -> dict:
    ...
    dominant_dist = max(distributions, key=lambda d: sum(matrix[m].get(d, 0) for m in matrix)) if results else "factual"
    dominant_metric = max(matrix.keys(), key=lambda m: sum(matrix[m].values())) if results else "faithfulness"

    diag, fix = DIAGNOSTIC_TREE.get(dominant_metric, ("Unknown failure", "Review pipeline configuration"))
    insight = (
        f"Distribution '{dominant_dist}' có nhiều failure nhất. "
        f"Metric '{dominant_metric}' ({diag}) là điểm yếu chủ đạo. "
        f"Gợi ý: {fix}"
    )
